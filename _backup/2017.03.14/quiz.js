...
function QuestionTemplate(qDomRef, qTitle, qType, qOptions) {
...
function questionButtonsClick(e, whichQ) {
...
function insertNewQuestion() {
	...
	newDiv.load('qTemplate.html', function(){
		...
		var newQObj = new QuestionTemplate($lastQuestion[0], "title", "mc", questionOptions);
		//sets newly created object as a subobject of the masterArray object with a key of the question's ID
		masterArray[$lastQuestion[0].id] = newQObj;
	});
}

//trigger gathering functions on blur of relevant input fields and update master object array
//create function for gathering question title
//create functions for setting question type
//create function for gathering question options
...
function deleteQuestion(e) {
	...
	//update masterArray
	delete masterArray[e.target.parentNode.parentNode.parentNode.parentNode.id];
}
